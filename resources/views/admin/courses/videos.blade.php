...
@section('content')
  ...
  <details class="card-rt mb-4">
    ...
    <form method="POST" action="{{ route('admin.courses.videos.store', $course) }}" enctype="multipart/form-data" class="mt-4 js-video-upload-form">
      @csrf
      <div class="row g-3">
      ...
      <p style="font-size:11px;color:var(--muted);margin-top:8px">MP4, WebM, OGG or MOV &middot; no size limit. Leave the file empty to save the video entry now and attach the file later by editing it. Premium videos are stored privately — only enrolled students, the owning franchisee and admins can stream them.</p>
  ...
  @foreach ($course->modules as $module)
    @if ($module->videos->count())
      <div class="card-rt mb-3">
        ...
        @include('admin.courses.partials.video-table', ['videos' => $module->videos, 'course' => $course])
      </div>
    @endif
  @endforeach
  ...
  @if ($unassigned->count())
    <div class="card-rt mb-3">
      ...
      @include('admin.courses.partials.video-table', ['videos' => $unassigned, 'course' => $course])
    </div>
  @endif
@endsection
