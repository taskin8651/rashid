...
@section('content')
  <a href="{{ route('admin.leads.index') }}" style="color:var(--muted);text-decoration:none;font-size:12px" class="d-inline-block mb-3"><i class="bi bi-arrow-left me-1"></i>Back to Leads</a>
  ...
  <div class="ov-banner">
    ...
    <div class="ov-banner-stats">
      ...
      <div><b>{{ $lead->course->name ?? '—' }}</b><span>Course Interested</span></div>
      <div><b>{{ $lead->assignedTo->name ?? 'Unassigned' }}</b><span>Assigned To</span></div>
      <div><b>{{ optional($lead->next_follow_up_date)->format('d M Y') ?? '—' }}</b><span>Next Follow-up</span></div>
    </div>
  </div>
  ...
  <div class="row g-4 mt-1">
    <div class="col-lg-7">
      <div class="card-rt mb-4">
        ...
        @else
          <form method="POST" action="{{ route('admin.leads.status.update', $lead) }}" class="d-flex gap-2 align-items-end mb-3 flex-wrap">
            @csrf
            <div>
          ...
          <div class="modal fade" id="lostReason" tabindex="-1">
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <form method="POST" action="{{ route('admin.leads.status.update', $lead) }}">
                  @csrf
    ...
    <div class="col-lg-5">
      @can('manage-leads')
        ...
        <form method="POST" action="{{ route('admin.leads.destroy', $lead) }}" onsubmit="return confirm('Delete this lead? This cannot be undone.')">
          @csrf @method('DELETE')
